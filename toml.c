...
static size_t f_err_str_len = sizeof(file_error_str)/sizeof(file_error_str[0]);
static char alloc_error_str[] = "error in memory allocation";
static size_t a_err_str_len = sizeof(alloc_error_str)/sizeof(alloc_error_str[0]);


struct toml_val *
toml_parse_file(FILE *fp, char *err, size_t err_len,
		void *(*falloc)(size_t size), void (*ffree)(void *ptr))
{
	size_t flen;
	char *str;

	if ((fseek(fp, 0, SEEK_END) < 0) || ((flen = ftell(fp)) < 0) || (fseek(fp, 0, SEEK_SET) < 0)) {
		strncpy(err, file_error_str, err_len < f_err_str_len ? err_len : f_err_str_len);
		return NULL;
	}
	if (!falloc) {
		falloc = &malloc;
	...
	if ((str = falloc(flen + 1)) == NULL) {
		strncpy(err, alloc_error_str, err_len < a_err_str_len ? err_len : a_err_str_len);
		return NULL;
	}
...
void free_toml_val(struct toml_val *v, void (*ffree)(void *ptr))
{
	...
	case TOML_TABLE:
		...
		(*ffree)(v->u.table.keys); /* individual keys are owned by subvals and freed with them */
	}
	(*ffree)(v->key);
	(*ffree)(v);
}
